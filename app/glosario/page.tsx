  ...
  return (
    <div className="group border-b border-border/60 py-5 last:border-0">
      <div className="flex items-start gap-4">
        {entrada.siglas ? (
          <span className="shrink-0 w-20 text-right font-mono text-sm font-bold text-green-400 pt-0.5 leading-relaxed">
            {entrada.siglas}
          </span>
        ) : (
        ...
        <div className="min-w-0">
          <p className="text-base font-semibold text-text leading-snug mb-1">
            {entrada.termino}
          </p>
          {entrada.definicion && (
...
export default async function GlosarioPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="mb-8 flex flex-wrap gap-1.5">
            {letras.map((letra) => (
              <a
                key={letra}
                href={`#letra-${letra}`}
                className="rounded border border-border bg-bg-surface px-2.5 py-1 font-mono text-sm text-text-muted hover:border-green-500/30 hover:text-green-400 transition-colors"
              >
                {letra}
              </a>
            ))}
          ...
          <div className="mb-8 flex flex-wrap gap-1.5">
            {categoriasPresentes.map((cat) => (
              <a
                key={cat}
                href={`#cat-${cat}`}
                className="rounded border border-border bg-bg-surface px-2.5 py-1 text-sm text-text-muted hover:border-green-500/30 hover:text-green-400 transition-colors"
              >
          ...
          <div className="space-y-10">
            {categoriasPresentes.map((cat) => (
              <section key={cat} id={`cat-${cat}`}>
                <h2 className="mb-3 text-base font-semibold text-green-400 uppercase tracking-widest scroll-mt-16">
                  {categoriaGlosarioLabel(cat)}
                  <span className="ml-2 text-text-muted/50 font-mono text-sm normal-case tracking-normal">
      ...
      <p className="mt-10 text-[13px] text-text-muted/50">
