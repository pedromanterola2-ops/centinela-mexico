...
function renderValor(valor: string | number | null) {
  if (valor === null) {
    return <span className="text-text-muted text-sm">Pendiente</span>;
  }
...
export function CompareTable({
  ...
  if (rows.length === 0) {
    return (
      ...
      >
        <p className="text-base">Selecciona dos elementos para comparar.</p>
      </div>
    );
  }
        ...
        <tbody>
          {rows.map((row, i) => (
            <tr key={i} className="border-b border-border last:border-0">
              <td className="py-2.5 px-4 text-text-muted text-sm uppercase tracking-wide">
                {row.campo}
              </td>
              <td
                className={cn(
              ...
              >
                ...
                {row.mayor === "A" && (
                  <span className="ml-1 text-xs text-green-400/70">▲</span>
                )}
              </td>
              <td
                className={cn(
              ...
              >
                ...
                {row.mayor === "B" && (
                  <span className="ml-1 text-xs text-guinda-300/70">▲</span>
                )}
              </td>
            </tr>
          ))}
