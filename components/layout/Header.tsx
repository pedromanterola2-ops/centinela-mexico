        ...
        <div className="flex h-12 items-center gap-0">
          ...
          <Link href="/" className="flex items-center gap-2.5 shrink-0 mr-4">
            <div className="flex h-6 w-6 items-center justify-center border border-green-500/40 bg-green-500/10 rounded-sm">
              <span className="text-[9px] font-bold text-green-400 font-mono tracking-wider">CM</span>
            </div>
            <div>
              <span className="text-[11px] font-bold text-[#e8f0ec] leading-none tracking-[0.05em] uppercase">
                Centinela
              </span>
              <span className="block text-[8px] text-text-muted leading-none tracking-[0.2em] uppercase mt-0.5">
                México
              </span>
            </div>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-0.5 flex-1 overflow-x-auto">
            {NAV_ITEMS.map((item) => (
              <Link
                ...
                href={item.href}
                className={cn(
                  "whitespace-nowrap rounded-sm px-2.5 py-1.5 text-[11px] font-medium transition-colors",
          ...
          <div className="flex items-center gap-3 ml-auto">
            ...
            <div className="hidden lg:flex items-center gap-1">
              {SECONDARY_NAV.map((item) => (
                <Link
                  key={item.href}
                  href={item.href}
                  className="text-[10px] text-text-muted/60 hover:text-text-muted transition-colors px-2 py-1"
                >
                  {item.label}
                </Link>
              ))}
            ...
            <div className="hidden lg:flex items-center gap-1.5 pl-3 border-l border-border">
              ...
              <span className="font-mono text-[8px] text-text-muted/50 tracking-[0.15em] uppercase">
                Fuentes abiertas
              </span>
            </div>
