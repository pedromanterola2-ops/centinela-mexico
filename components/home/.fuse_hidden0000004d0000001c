    ...
    <div className="mx-auto max-w-2xl px-4">
      <form onSubmit={handleSubmit} className="relative">
        <div className="relative">
          ...
          <button
            type="submit"
            className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md bg-green-500/10 border border-green-500/20 text-green-400 px-3 py-1.5 text-xs font-medium hover:bg-green-500/20 transition-colors"
          >
            Buscar
          </button>
        </div>
          ...
          <div className="absolute z-20 mt-1 w-full rounded-lg border border-border bg-bg-elevated shadow-xl">
            {filtered.map((s) => (
              <button
                ...
                className="w-full text-left px-4 py-2.5 text-sm text-text-muted hover:text-text hover:bg-bg-surface transition-colors first:rounded-t-lg last:rounded-b-lg"
              >
